/* =====================================================
   PROSAGO WORKS — UI Interactions (Vanilla JS)
   ===================================================== */
  ...
  document.querySelectorAll('.needs-validation').forEach(form => {
    form.addEventListener('submit', function (e) {
      if (!form.checkValidity()) {
        ...
        e.preventDefault();
        const existingNotice = form.querySelector('.form-status');
        const notice = existingNotice || document.createElement('p');
        notice.className = 'form-status mt-3 mb-0';
        notice.setAttribute('role', 'status');
        notice.textContent = 'This form is a placeholder and is not connected yet. Please add the approved contact destination before launch.';
        if (!existingNotice) form.appendChild(notice);
      }
